...
def plot_hsp_3d_streamlit(candidate_df: pd.DataFrame) -> None:
    """
    Streamlit-safe 3D Hansen Solubility Parameter plot for candidate materials.
    Expects columns: name, dD, dP, dH.
    """
    import matplotlib.pyplot as plt
    from mpl_toolkits.mplot3d import Axes3D  # noqa: F401

    plot_df = candidate_df.dropna(subset=["dD", "dP", "dH", "name"]).copy()
    if plot_df.empty:
        st.info("No valid HSP data available for 3D plot.")
        return

    # Small figure + high DPI = crisp, but avoid being so tiny text becomes unreadable
    fig = plt.figure(figsize=(6.0, 5.0), dpi=600)
    ax = fig.add_subplot(111, projection="3d")

    #ax.scatter(plot_df["dD"], plot_df["dP"], plot_df["dH"], s=7, alpha=1.0)
    ax.scatter(plot_df["dP"], plot_df["dH"], plot_df["dD"], s=6, alpha=1.0)
    ...
    # Axis labels (tiny)
    """ax.set_xlabel("δD", fontsize=6, fontweight="bold")
    ax.set_ylabel("δP", fontsize=6, fontweight="bold")
    ax.set_zlabel("δH", fontsize=6, fontweight="bold", labelpad=10)"""
    ...
    # Tick label size (THIS controls 15, 17.5, 20...)
    ...
    #ax.set_title("Candidate Materials in HSP Space", fontsize=3, pad=2)

    # Tight layout with minimal padding
    plt.tight_layout(pad=0.05)

    fig.subplots_adjust(left=0.02, right=0.95, bottom=0.02, top=0.90)
    # (skip tight_layout for 3D)

    # 🚫 Key: do NOT let Streamlit stretch it to container width
...
def main():
    ...
    if st.button("Calculate Similarity"):
        ...
        plot_hsp_3d_streamlit(materials_df)
