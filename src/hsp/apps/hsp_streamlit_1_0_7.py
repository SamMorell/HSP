...
TARGET_POLYMER_TYPES_XLSX = "Polymer_Types.xlsx"
...
def plot_hsp_3d_streamlit(candidate_df: pd.DataFrame, results_df: pd.DataFrame, target: dict) -> None:
    """
    Interactive 3D HSP plot using Plotly (mouse-rotatable in Streamlit).

    - Target plotted in black (center of sphere)
    - Candidates with distance <= 3 plotted in green
    - Candidates with 3 < distance <= 10 plotted in blue
    - Candidates with distance > 10 plotted in red
    - A slightly transparent sphere is drawn around the blue+green points (distance <= 10)
      with radius equal to the maximum Euclidean distance from target to any distance<=10 candidate.
    """
    import numpy as np
    import plotly.graph_objects as go

    # ---- Robustly identify columns in results_df (raw vs display-renamed) ----
    ...
    # Coordinates (match your current axis order): x=δP, y=δH, z=δD
    x = plot_df["dP"].astype(float)
    y = plot_df["dH"].astype(float)
    z = plot_df["dD"].astype(float)

    dist = plot_df["distance"].astype(float)

    #blue = dist <= 8
    green = dist <= 3
    #green = (dist > 8) & (dist <= 10)
    blue = (dist > 3) & (dist <= 10)
    red = dist > 10

    # Target center
    cx, cy, cz = float(target["dP"]), float(target["dH"]), float(target["dD"])

    # Sphere radius based on max Euclidean distance among distance<=10
    within_10 = dist <= 10
    sphere_pts = plot_df.loc[within_10, ["dP", "dH", "dD"]].astype(float)

    r = 0.0
    if not sphere_pts.empty:
        r = float(np.sqrt(
            (sphere_pts["dP"] - cx) ** 2 +
            (sphere_pts["dH"] - cy) ** 2 +
            (sphere_pts["dD"] - cz) ** 2
    ...
    if r > 0:
        ...
        fig.add_trace(go.Surface(
            ...
            opacity=0.12,
            showscale=False,
            name="Radius (≤10) Sphere",
            hoverinfo="skip"
        ))

    # Candidates (use hover + optional text labels)
    def add_candidates(mask, name, color):
        if mask.sum() == 0:
            return
    ...
    #add_candidates(blue,  "Distance ≤ 8",  "blue")
    add_candidates(green,  "Distance ≤ 3",  "green")

    #add_candidates(green, "8 < Distance ≤ 10", "green")
    add_candidates(blue, "3 < Distance ≤ 10", "blue")

    add_candidates(red,   "Distance > 10", "red")

    # Target (black)
    fig.add_trace(go.Scatter3d(
        x=[cx], y=[cy], z=[cz],
        mode="markers+text",
        text=[str(target.get("name", "Target"))],
        textposition="top center",
        textfont=dict(size=12, color="black"),
        marker=dict(size=6, color="black", opacity=1.0),
        name="Target",
        hovertemplate=(
            "<b>%{text}</b><br>"
            "δP=%{x:.2f}<br>"
...
def load_target_catalog() -> pd.DataFrame:
    ...
    Build a unified catalog from:
      ...
      Polymer_Types.xlsx (polymer-family tabs, preserving tab + row order)
    ...
    # --- Polymer_Types.xlsx ---
    p = _find_first_existing(_candidate_locations(TARGET_POLYMER_TYPES_XLSX))
    if p:
        for sh_order, sh, df in _read_excel_all_sheets(p):
            df = _safe_clean(_safe_normalize(df))
            df = df.reset_index(drop=True)
...
def plot_hsp_3d_matplotlib_fallback(candidate_df: pd.DataFrame, results_df: pd.DataFrame, target: dict) -> None:
    """
    Static matplotlib fallback when WebGL / Plotly 3D is unavailable.
    """
    ...
    dist_col = "distance" if "distance" in results_df.columns else "Distance"

    merge_df = results_df[[name_col, dist_col]].rename(columns={name_col: "name", dist_col: "distance"})
    plot_df = candidate_df.merge(merge_df, on="name", how="left").dropna(subset=["dD", "dP", "dH"])
    ...
    #ax.scatter(x[d <= 8], y[d <= 8], z[d <= 8], s=6, c="blue")
    ax.scatter(x[d <= 3], y[d <= 3], z[d <= 3], s=6, c="green")

    #ax.scatter(x[(d > 8) & (d <= 10)], y[(d > 8) & (d <= 10)], z[(d > 8) & (d <= 10)], s=6, c="green")
    x.scatter(x[(d > 3) & (d <= 10)], y[(d > 3) & (d <= 10)], z[(d > 3) & (d <= 10)], s=6, c="blue")

    ax.scatter(x[d > 10], y[d > 10], z[d > 10], s=6, c="red")

    ax.scatter(target["dP"], target["dH"], target["dD"], s=40, c="black")
...
def main():
    ...
    if target_mode == "Pick from library":
        ...
        if _find_first_existing(_candidate_locations(TARGET_POLYMER_TYPES_XLSX)) is None:
            missing_msgs.append(f"- {TARGET_POLYMER_TYPES_XLSX} (tabs remain; first column: Material)")
        ...
        if missing_msgs and catalog.empty:
            ...
            browse_by = st.selectbox("Browse by", ["Materials", "Material Types", "Polymer Types", "Suppliers"])
            subset = catalog.copy()
            ...
            elif browse_by == "Polymer Types":
                subset = subset[subset["source_workbook"] == TARGET_POLYMER_TYPES_XLSX]
                if subset.empty:
                    st.warning(f"No rows found from {TARGET_POLYMER_TYPES_XLSX}.")
                else:
                    tabs_df = subset[["source_sheet", "sheet_order"]].drop_duplicates()
                    tabs_df = tabs_df.sort_values("sheet_order", kind="stable")
                    tab_options = tabs_df["source_sheet"].astype(str).tolist()
    ...
    if st.button("Calculate Similarity"):
        ...
        try:
            ...
            st.warning("Interactive 3D unavailable (WebGL). Showing static fallback.")
            plot_hsp_3d_matplotlib_fallback(materials_df, results_df, target)
