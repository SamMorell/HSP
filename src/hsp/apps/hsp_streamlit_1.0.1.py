"""
hsp_streamlit_1.0.1.py

Changes in v1.0.1 (per Sam's step-by-step plan):
...
- Suppliers workbook behavior unchanged for now (will adjust in a later step).
...
st.caption("RUNNING: hsp_streamlit_1.0.1.py")
...
if target_mode == "Pick from library":
    ...
    if missing_msgs and catalog.empty:
        ...
        if browse_by == "Suppliers":
            # Keep existing behavior for now (alphabetical is OK until we refactor Suppliers.xlsx)
            ...
            product = st.selectbox("Material", sorted(subset["product"].unique()))
