"""
hsp_streamlit_1.0.2.py

Changes in v1.0.2 (per Sam's step-by-step plan):
- App title: "Hansen Solubility Parameters"
- Section order: Target Materials → Candidate Materials → Results
- Section names simplified: Target Materials / Candidate Materials / Results
- Target library workbook naming updates:
    * Products.xlsx → Materials.xlsx (sheet: "Materials", first column: "Material")
    * Product_Types.xlsx → Material_Types.xlsx (tabs remain; first column renamed to "Material")
- Dropdown ordering:
    * Materials dropdown preserves the EXACT row order from Materials.xlsx (no alphabetical sorting)
    * Material Types tabs preserve workbook tab order; materials preserve row order within each tab
- Suppliers workbook now uses columns: Material, Supplier (preserves workbook tab order and material order as saved).

Data folders:
- Target dropdowns driven by local Excel files in: data/target_materials/
- Optional default candidate dataset in: data/target_materials/Default_Materials.xlsx (or data/candidates/ as fallback)
"""
...
GIF_PATH = Path(__file__).resolve().parents[3] / "assets" / "sammorell.com_animated_header.gif"
...
# Updated workbook names (step 1)
TARGET_MATERIALS_XLSX = "Materials.xlsx"
TARGET_MATERIAL_TYPES_XLSX = "Material_Types.xlsx"
TARGET_SUPPLIERS_XLSX = "Suppliers.xlsx"  # unchanged (future step)
...
def _safe_normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    IMPORTANT:
    - We must rename Material/Product → name BEFORE calling the core validator,
    ...
    df = df.rename(
        columns={
            ...
            "Material": "name",
            "Product": "name",
            # Candidate uploads / legacy aliases
            "Name": "name",
            "Solvent": "name",
            "Solvent/Name": "name",
            # HSP aliases
...
def load_target_catalog() -> pd.DataFrame:
    ...
    Build a unified catalog from:
      ...
      Suppliers.xlsx (unchanged for now)

    Returns:
      product, product_type, supplier, dD, dP, dH, source_workbook, source_sheet,
      sheet_order, row_order, notes
    """
    ...
    if p:
        # Prefer sheet "Materials", else first sheet
        try:
        ...
        # Preserve the row order as it appears in the workbook
    ...
    # --- Material_Types.xlsx (tabs = material types) ---
        ...
        for sh_order, sh, df in _read_excel_all_sheets(p):
            ...
            rows.append(
                pd.DataFrame(
                    {
                        "product": df["name"],
                        "product_type": sh,  # tab name = material type
...
#st.title("Hansen Solubility Parameters")
st.markdown(
    '<div class="hsp-title">Hansen Solubility Parameters</div>',
    unsafe_allow_html=True
)

st.caption(f"v{APP_VERSION}")
st.caption("RUNNING: hsp_streamlit_1.0.2.py")

# ===== 2) Target Materials (moved to top) =====
#st.header("Target Materials")
st.markdown(
    '<div class="hsp-section">Target Materials</div>',
    unsafe_allow_html=True
)
...
if target_mode == "Pick from library":
    # Check for the two workbooks in this step
    ...
    if missing_msgs and catalog.empty:
        st.warning(
            ...
            + "\n".join(missing_msgs)
            + f"\n- {TARGET_SUPPLIERS_XLSX} (unchanged for now)"
        )
    elif catalog.empty:
        st.warning(
            f"Target catalog is empty. Check your Excel files under ./data/{TARGET_DIRNAME}/."
        )
    else:
        ...
        if browse_by == "Suppliers":
            # Only rows from Suppliers.xlsx — preserve:
            #   - supplier order (first-seen order)
            #   - workbook tab order (sheet_order)
            #   - material order within each tab (row_order)
            subset = subset[subset["source_workbook"] == TARGET_SUPPLIERS_XLSX]
            if subset.empty:
                st.warning(f"No rows found from {TARGET_SUPPLIERS_XLSX}.")
            else:
                # Sort once by workbook order then row order so first-seen order is meaningful
                ...
                subset_tab = subset_supplier[subset_supplier["source_sheet"] == tab].sort_values("row_order", kind="stable")
            ...
            # Only rows from Material_Types.xlsx
            ...
            if subset.empty:
                ...
                # Preserve workbook tab order (no sorting)
            ...
            # Materials (from Materials.xlsx) — preserve Excel row order
...
# ===== 1) Candidate Materials (moved below target) =====
#st.header("Candidate Materials")
st.markdown(
    '<div class="hsp-section">Candidate Materials</div>',
    unsafe_allow_html=True
)
...
#st.header("Results")
st.markdown(
    '<div class="hsp-section">Results</div>',
    unsafe_allow_html=True
)

materials_df = None
if uploaded_file is not None:
    # Read the uploaded Excel ourselves so we can normalize "Material/Product" -> "name"
